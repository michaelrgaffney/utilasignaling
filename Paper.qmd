...
Caregivers were asked various questions about their home, which we used to create a home quality index. This involved scoring the following summing the affrimatives to the following conditions: having an indoor bathroom, having a bathroom that is not shared with anyone outside of the household, having a flush toilet, having running water to wash hands in the bathroom, and having a home which did not flood in the last year.

Caregiver perceptions of neighborhood quality were measured using ten questions on a five point Likert scale relating to the costs and benefits associated with living in one's neighborhood. The mean of these values was used in analyses as a neighborhood quality index. See @stbl-Neighborhood-Quality for variable wording.

Levels of home instability were measured with four questions covering the degree to which the caregiver's family had/has to move homes and whether people move in and out of their homes frequently. All questions invovled five point Likert scales. See @stbl-Home-Instability for variable wording.
...
To investigate the associations between potential causes of child signaling and conflict with caretakers and caregiver reports of child behavior, we used elasticnet regression models. Elasticnet models (e.g., lasso and ridge regression) are well suited to instances when the number of predictor variables are large relative to the number of observations due their use of penalty terms and cross-validation to limit the risk of overfitting the data.

Analyses were run in R using the glmnet package [@friedman2010] with a quasipoisson error family due to our outcome variables being overdispered counts. Predictors includeddemographic, household, health, and neighborhood variables that might cause signaling behavior or conflict between children and caretakers. All variables were centered and standardized by one standard deviation prior to fitting.

Parental responses to child signaling, their perceptions of child need, and their reported investment in the child were coded as three-level ordinal variables, either negative, neutral or positive; or less than other children, same as other children, or more than other children. We analyzed these outcomes using ordinal logistic regression with elastic net penalties, fit in R using the ordinalNet package [@wurm].
...
# dx <- modeldf
# names(dx) <- shortform_dict[names(modeldf)] # set-names??? line 1844
# datasummary_skim(dx, type = "categorical)
```
...
::: {#fig-effect_last_signal_response2}
```{r, fig.width= 8}
...
[needs updating. worth having a section just for this?]
We found no evidence of associations between sex and child signaling behavior outside of the previously mentioned interactions. Monthly household income also was not a significant predictor of signaling behavior in any model nor was population when using language, immigration status, and neighborhood as proxies for it. 
...
\[delete?\] *In most models, there is a negative association between education and child signaling?*
...
Variable names and question wording for measures about a child's health history. Both questions were asked using a Likert scale from 1-5 where 1 was labeled "Strongly disagree" and 5 was labeled "Strongly agree." {#stbl-medical-record}
...
: Variable names and question wording for the modified version of the USDA Food Security Questionnaire [@blumberg1999]. All questions were asked using a Likert scale from 1-5 where 1 was labeled “Strongly disagree” and 5 was labeled “Strongly agree.” {#stbl-Food-Security}
...
: Variable names and question wording for the home instability questions. All questions were asked using a Likert scale from 1-5 where 1 was labeled “Disagree strongly” and 5 was labeled “Agree Strongly.” {#stbl-Home-Instability}
...
: Variable names and question wording for the perceptions of neighborhood quality questions. All questions were asked using a Likert scale from 1-5 where 1 was labeled “Strongly disagree” and 5 was labeled “Strongly agree.” {#stbl-Neighborhood-Quality}
...
: List of options for research assistant to classify participant responses to the question of how they responded when their child was last sad or cried. {#stbl-response-valence}

List of possessions owned (yes or no).

Motorized vehicle (scooter, 4-wheeler, etc.) Bicylce Television (TV) with cable Smartphone Washing machine Refrigerator A home that you own Credit card
