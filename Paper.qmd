...
The overall structure of study variables was analyzed with principle components analysis (PCA) and Bayesian graphical models (BGM) and minimum spanning trees (MST).
...
Lasso regression models of each child signaling frequency and conflict frequency. Variables whose coefficient values were shrunk to 0 are not displayed. Models fit uins using quasi-Poisson regression.
:::

The effects of particularly large risk or protective factors for child signaling are shown in @fig-signal-effects.

::: {#fig-signal-effects}
![](Figures/signal_effects_plot.svg)

Selected predictors of child signaling frequencies.
:::
...
Selected predictors of child-parent conflict frequencies.
:::
...
plot_need_age / plot_need_sad + ggtitle("") + plot_layout(guides = 'collect') + plot_annotation(tag_levels = "A")
```
...
plot_caregiver_punish / plot_caregiver_pain + ggtitle("") + plot_layout(guides = 'collect')
```
...
As seen in Western cultures [@zeifman2013; @potegal2003; @koch2003] (check for others), the frequencies of crying and temper tantrums decreased with age, as did the frequency of child sadness, summed signal frequency, and signal cost. One possible reason for this is that as children mature, they are more capable of fulfilling their own needs, more able to ask others for what they need, and are less vulnerable to threats. Another is that since one's reproductive value increases as one survives periods of high mortality risk in childhood, the intensity of conflicts of interest with parents decreases since these children are likely to reach adulthood and require less investment to reach adulthood than younger children, current or future [@daly1995]. For this reason, less costly signals may be more likely to work to increase investment, all else being equal\[\[4\]\]. That said, all else may often not be equal if investment in older children is costlier, something which may be reflected in caretakers seeing older children as relatively needy. The decrease in crying and tantrum frequencies with age may also result from the potential for greater reputational costs, something which may also increase the credibility of such signals among older children as previously mentioned.
...
[delete based on lasso coefficients?]
